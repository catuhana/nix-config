            ...
            homeManager =
              { pkgs, lib, ... }:
              {
                programs.gnome-shell.extensions =
                  lib.optionals user.caden.home.gnome.shell.extensions.blur-my-shell.enable [
                    { package = pkgs.gnomeExtensions.blur-my-shell; }
                  ]
                  ++ lib.optionals user.caden.home.gnome.shell.extensions.caffeine.enable [
                    { package = pkgs.gnomeExtensions.caffeine; }
                  ]
                  ++ lib.optionals user.caden.home.gnome.shell.extensions.appindicator.enable [
                    { package = pkgs.gnomeExtensions.appindicator; }
                  ]
                  ++ lib.optionals user.caden.home.gnome.shell.extensions.copyous.enable [
                    {
